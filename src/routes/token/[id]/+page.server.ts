...
export async function load(event: RequestEvent): Promise<{
    ...
    const txs: ApiResponse<PagedModel<TokenTransaction>> = await callApi(event, 'GET', `/v1/tokens/${event.params["id"]}/token-transactions?sort=timestamp,desc&` + event.url.searchParams.toString())
    return {token, txs};
}